$('#lefile').fileinput({//初始化上传文件框
    ...
    //上传文件成功，回调函数 
    $('#lefile').on("fileuploaded", function(event, data, previewId, index) {
    ...
    */
    $.ajax({//上传文件成功后再保存图片信息
        ...
        type:'post',
        dataType:'json',
        data:$('#fileform').serialize(),//form表单的值
        success:function(data,status){
            if (status == "success") {
    ...
    $('.apply').on('click',function (){// 提交图片信息 //
        //先上传文件，然后在回调函数提交表单
        $('#lefile').fileinput('upload');
