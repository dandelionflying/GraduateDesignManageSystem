...
function getGrades(index,size){
	$.ajax({
		type : "GET",
		url : "getGradesByClass/"+index+"/"+size,
	...
	if(result.total==0){
		...
		$.each(rows,function(i,item){
			var $tr = $("<tr class='warning'>" +
					...
					"<td id='grade-translate'>"+item.translate+"</td>" +
					"<td id='grade-evaluation-"+item.id+"'>"+item.evaluation+"</td>" +
					"<td id='grade-guider'>"+item.guider+"</td>" +
...
function showFrom(id){
	var $gradetd = $("#grade-evaluation-"+id);
	var grade = $gradetd.text();
	$("#btn-addgrade").hide();
	$gradetd.html("<input type='text' id='new-grade' class='form-control' placeholder='grade' value='"+grade+"'>");
	$("#options-"+id).append(" <button class='btn btn-success btn-xs' id='btn-submit' disabled='disabled'>提交</button> " +
			"<button class='btn btn-danger btn-xs' id='btn-cancel' onclick='cancel("+grade+","+id+")'>取消</button>");
	
	$("#new-grade").on('blur',function(){
		var grade = $(this).val();
		console.log(grade);
		checkFrom(grade);
	});
...
function cancel(grade,id){
	$("#new-grade").remove();
	$("#grade-evaluation-"+id).html(grade);
	$("#btn-addgrade").show();
	$("#btn-cancel").remove();
	$("#btn-submit").remove();
}
