...
function changePage(result,index,size){
	if(result.total>0){
		...
		$.each(result.rows,function(i,item){
			time = timeFormatter(item.uploadTime);
			var li = $("<li id='file-item-"+i+"'><div class='task-checkbox'>"+
					"<input type='hidden' value='1' name='test'>"+
					"<input type='checkbox' class='liChild' value='"+item.id+"' name='mubanfile'> </div>"+
					"<div class='task-title' id='title-"+i+"'>"+
					"<span class='task-title-sp'>"+item.name+"</span>" +
					"<span style='float:right;font-style: italic;'>"+time+"</span>" +
					"</div>" +
			"</li>");
			var $span1 = item.receiver==0?$("<span class='label label-sm label-all'>全部</span>"):
				(item.receiver==1?$("<span class='label label-sm label-student'>学生</span>"):$("<span class='label label-sm label-teacher'>教师</span>"));
			fileul.append(li);
			...
			$("#file-item-"+i).append("<div class='task-config'>" +
					...
					"<li id='item-delete-"+item.id+"' ><a href='javascript:;'><i class='am-icon-trash-o'></i> 删除 </a></li>" +
					"</ul>" +
					"</div>" +
			"</div>");
			$("#item-delete-"+item.id).on('click',function(){
//				console.log(item.id);
				deletemubanfile(item.id);
			});
		});
	}else{
		var fileul = $(".tpl-task-list");
...
window.onload = function(){
	...
	$("#btn-delete").on('click',function(){
		var checks = [];
		$('input[name="mubanfile"]:checked').each(function(){
			checks.push($(this).val());
		});
		$.ajax({
			type : "POST",
			url : "deleteMubanFiles",
			data : {
				"ids" : checks
			},
